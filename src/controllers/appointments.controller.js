...
exports.createAppointment = async (req, res) => {
  ...
  const appointmentData = {
    ...
    instructureProfileId: data.instructureProfileId,
    instructureName: data.instructureName,
  };
  ...
  if (appointmentData) {
    ...
    if (oldAppointment.length) {
      ...
      const emailData = {
        ...
        instructureProfileId: appointmentData.instructureProfileId,
        topics: topics,
        slug: slug,
  ...
  if (data.appointmentId) {
    ...
    if (isUpdate) {
      await utils.cancelAppointmentNotificationMail(
        data.profileId,
        data.instructureName
      );
      await utils.cancelAppointmentNotificationMail(
        data.instructureProfileId,
        data.instructureName
      );
      res.send({
        error: false,
